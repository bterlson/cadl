...
export async function list(client: TodoContext, options?: {
"limit"?: number;
"offset"?: number;
...
export async function create(client: TodoContext, item: TodoItem, contentType: "application/json", options?: {
...
"labels"?: TodoLabels;
  ...
  const httpRequestOptions = {
    ...
    body: JSON.stringify({
      ...
      "attachments": options?.attachments ? arraySerializer(options?.attachments, ) : options?.attachments
    }),
  };
  ...
  if (response.status === 200) {
    ...
    return {
      ...
      "completedAt": bodyJson.completedAt ? dateDeserializer(bodyJson.completedAt) : bodyJson.completedAt,
...
export async function update(client: TodoContext, id: number, patch: TodoItemPatch, contentType: "application/merge-patch+json"): Promise<{
"id": number;
"title": string;
"createdBy": number;
"assignedTo"?: number;
